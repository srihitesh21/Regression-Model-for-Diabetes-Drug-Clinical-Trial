...
def create_tf_categorical_feature_cols(categorical_col_list,
    ...
    for c in categorical_col_list:
        ...
        catvocab = tf.feature_column.categorical_column_with_vocabulary_file(c, vocab_file_path)
        tf_categorical_feature_column = tf.feature_column.embedding_column(catvocab, dimension=10)
...
def create_tf_numeric_feature(col, MEAN, STD, default_value=0):
    ...
    '''
    norm = functools.partial(normalize_numeric_with_zscore, mean=MEAN, std=STD)
    tf_numeric_feature = tf.feature_column.numeric_column(
        key= col, default_value=default_value, dtype=tf.float64, normalizer_fn=norm)
